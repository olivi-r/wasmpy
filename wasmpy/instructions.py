...
def read_memarg(buffer):
    a = read_uint(buffer, 32)
    o = read_uint(buffer, 32)
    return {"align": a, "offset": o}


def read_blocktype(buffer):
    data = buffer.read(1)
    if data == b"\x40":
        return None

    if data in b"\x7f\x7e\x7d\x7c":
        return data

    # reconsume as 33 bit signed int
    buffer.seek(-1, 1)
    x = read_sint(buffer, 33)
    if x >= 0:
        return x
...
def read_instruction(buffer):
    data = buffer.read(1)
    # 0x0B (end) and 0x05 (else)
    if data == b"\x0b":
        return ["end"]
    ...
    if data == b"\x02":
        bt = read_blocktype(buffer)
        in_ = []
        while (instruction := read_instruction(buffer)) != ["end"]:
            in_.append(instruction)

        return ["block", bt, in_, "end"]
